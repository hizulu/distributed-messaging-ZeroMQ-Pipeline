...
class Inbox:
    ...
    def insert(self, data):
        ...
        unix_timestamp = data['occur_at'] if 'occur_at' in data else int(
            time.time())
        first_time_occur_at = data['first_time_occur_at'] if 'first_time_occur_at' in data else datetime.now(
        ).strftime('%Y-%m-%d %H:%M:%S')
        dttime = datetime.datetime.utcfromtimestamp(
            unix_timestamp).strftime('%Y-%m-%d %H:%M:%S')
        sql = """
            insert into tb_sync_inbox(row_id, table_name, msg_type, msg_id, query, client_unique_id, master_status, occur_at, first_time_occur_at, created_at, updated_at, priority)
            values("{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}", "{}")
        """

        return self.db.executeCommit(sql=sql.format(row_id, table_name, msg_type, msg_id, query, client_unique_id, master_status, unix_timestamp, first_time_occur_at, dttime, dttime, priority))
